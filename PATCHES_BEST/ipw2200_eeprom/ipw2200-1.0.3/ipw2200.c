...
void ipw_led_link_on(struct ipw_priv *priv)
{
	...
	if (priv->config & CFG_NO_LED ||
	    priv->nic_type == EEPROM_NIC_TYPE_1)
		return;
...
static ssize_t show_stats(struct device *d, char *buf)
{
	...
	u32 len = 0;
	len += STAT_PRINT(buf, last_rx_err, "%d");
	return len;
}
			      ...
			      struct ipw_associate *associate)
{
	...
	tmp_associate.policy_support = cpu_to_le16(tmp_associate.policy_support);
	tmp_associate.assoc_tsf_msw = cpu_to_le32(tmp_associate.assoc_tsf_msw);
	tmp_associate.assoc_tsf_lsw = cpu_to_le32(tmp_associate.assoc_tsf_lsw);
	tmp_associate.capability = cpu_to_le16(tmp_associate.capability);
	tmp_associate.listen_interval = cpu_to_le16(tmp_associate.listen_interval);
	tmp_associate.beacon_interval = cpu_to_le16(tmp_associate.beacon_interval);
				  ...
				  struct clx2_tx_queue *txq)
{
	...
	for (i = 0; i < le32_to_cpu(bd->u.data.num_chunks); i++) {
		pci_unmap_single(dev, le32_to_cpu(bd->u.data.chunk_ptr[i]),
				 le16_to_cpu(bd->u.data.chunk_len[i]), PCI_DMA_TODEVICE);
				       ...
				       struct ipw_rx_notification *notif)
{
	...
	case HOST_NOTIFICATION_STATUS_ASSOCIATED: {
		...
		case CMAS_INIT: {
			if (priv->status & STATUS_AUTH) {
				...
				resp = (struct ieee80211_assoc_response_frame *)&notif->u.raw;
				IPW_DEBUG(IPW_DL_NOTIF | IPW_DL_STATE | IPW_DL_ASSOC,
					  "association failed (0x%04X): %s\n",
					  ntohs(resp->status),
					  ipw_get_status_code(ntohs(resp->status)));
			}
	...
	case HOST_NOTIFICATION_NOISE_STATS: {
		if (notif->size == sizeof(u32)) {
			priv->last_noise = (u8)(le32_to_cpu(notif->u.noise.value) & 0xff);
...
static int ipw_find_adhoc_network(
        ...
        int roaming)
{
        ...
        if (network->time_stamp[0] < match->network->time_stamp[0]) {
                IPW_DEBUG_MERGE("Network '%s excluded because newer than current network.\n",
...
static void ipw_merge_adhoc_network(void *data)
{
       ...
       if ((priv->status & STATUS_ASSOCIATED) && (priv->ieee->iw_mode == IW_MODE_ADHOC)) {
				      ...
				      struct ieee80211_network *network)
{
	...
	case IEEE80211_52GHZ_BAND: /* A only */
		...
		if (priv->rates_mask & ~IEEE80211_OFDM_RATES_MASK) {
			...
			IPW_DEBUG_WX("invalid fixed rate mask in ipw_set_fixed_rate\n");
			fr.tx_rates = 0;
			break;
		}
...
//		ipw_abort_scan(priv);
	...
	scan.dwell_time[IPW_SCAN_ACTIVE_BROADCAST_AND_DIRECT_SCAN] = cpu_to_le16(20);
	...
	if (priv->ieee->iw_mode == IW_MODE_MONITOR) {
		...
		scan.dwell_time[IPW_SCAN_PASSIVE_FULL_DWELL_SCAN] = cpu_to_le16(2000);
	} else {
				 ...
				 int roaming)
{
	...
	if (priv->ieee->wpa_ie_len) {
		...
		ipw_set_rsn_capa(priv, priv->ieee->wpa_ie, priv->ieee->wpa_ie_len);
	}
	...
	if ((network->capability & WLAN_CAPABILITY_SHORT_PREAMBLE)
		...
		priv->assoc_request.capability &= ~WLAN_CAPABILITY_SHORT_PREAMBLE;
	}
				    ...
				    struct ieee80211_hdr *header)
{
	...
	case IW_MODE_ADHOC: /* Header: Dest. | Source    | BSSID */
		...
		return !memcmp(header->addr1, priv->net_dev->dev_addr, ETH_ALEN);
...
static void ipw_rx(struct ipw_priv *priv)
{
	...
	while (i != r) {
		...
		case RX_FRAME_TYPE: /* 802.11 frame */ {
			struct ieee80211_rx_stats stats = {
				.rssi = le16_to_cpu(pkt->u.frame.rssi_dbm) -
				IPW_RSSI_TO_DBM,
				.signal = le16_to_cpu(pkt->u.frame.signal),
				.noise = le16_to_cpu(pkt->u.frame.noise),
			...
			if (le16_to_cpu(pkt->u.frame.length) < frame_hdr_len(header)) {
				     ...
				     (WLAN_FC_GET_STYPE(le16_to_cpu(header->frame_ctl)) ==
				      IEEE80211_STYPE_BEACON))) {
						...
						struct ieee80211_probe_response * beacon;
                                                beacon = (struct ieee80211_probe_response *) header;
						if (le16_to_cpu(beacon->capability) & WLAN_CAPABILITY_IBSS) {
				   ...
				   union iwreq_data *wrqu, char *extra)
{
	...
	if (mode == 1) {
		if (!(priv->config & CFG_PREAMBLE_LONG)) {
			...
			/* Network configuration changed -- force [re]association */
			IPW_DEBUG_ASSOC("[re]association triggered due to preamble change.\n");
			if (!ipw_disassociate(priv))
				ipw_associate(priv);
		}
		goto done;
...
static inline void ipw_tx_skb(struct ipw_priv *priv, struct ieee80211_txb *txb)
{
	...
	for (i = 0; i < le32_to_cpu(tfd->u.data.num_chunks); i++) {
		...
		tfd->u.data.chunk_ptr[i] = cpu_to_le32(
			pci_map_single(
				priv->pci_dev,
				txb->fragments[i]->data + hdr_len,
				txb->fragments[i]->len - hdr_len,
				PCI_DMA_TODEVICE));
		tfd->u.data.chunk_len[i] = cpu_to_le16(
			txb->fragments[i]->len - hdr_len);
	}
	...
	if (i != txb->nr_frags) {
		...
		if (skb != NULL) {
			...
			txb->fragments[i] = skb;
			tfd->u.data.chunk_ptr[i] = cpu_to_le32(pci_map_single(
				priv->pci_dev, skb->data,
				tfd->u.data.chunk_len[i], PCI_DMA_TODEVICE));

			tfd->u.data.num_chunks = cpu_to_le32(
				le32_to_cpu(tfd->u.data.num_chunks) + 1);
		}
	}
...
static int ipw_setup_deferred_work(struct ipw_priv *priv)
{
	...
	INIT_WORK(&priv->led_link_on, (void (*)(void *))ipw_bg_led_link_on, priv);
	INIT_WORK(&priv->led_link_off, (void (*)(void *))ipw_bg_led_link_off, priv);
	INIT_WORK(&priv->led_act_off, (void (*)(void *))ipw_bg_led_activity_off, priv);
	INIT_WORK(&priv->merge_networks, (void (*)(void *))ipw_merge_adhoc_network, priv);
